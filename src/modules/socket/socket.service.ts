...
export class SocketService {
  ...
  async ready(baseUrl: string, timeout = 1000 * 60 * 5, retry = 5000): Promise<undefined> {
    try {
      ...
      this.logger[(error.response?.status || 500) < 500 ? 'warn' : 'error'](error);
    ...
    try {
          ...
          [
            {
              ...
              value: Object.entries({
                ...(secret.stringData || {}),
                ...Object.entries(secret.data || {}).reduce(this.base64DecodeMap, {}),
                ...secretStringData,
      ...
      const secret = (
        await this.coreV1Api.createNamespacedSecret(ns, {
          ...
          data: Object.entries(secretStringData).reduce(this.base64EncodeMap, {}),
        })
      ).body;
      this.logger.log(
  ...
  getOwnerReference(owner: k8s.KubernetesObject, childNamespace: string): k8s.V1OwnerReference {
  ...
  private base64EncodeMap(data: Record<string, string>, [key, value]: [string, string]): Record<string, string> {
    data[key] = Buffer.from(value, 'utf-8').toString('base64');
    return data;
  }

  private base64DecodeMap(stringData: Record<string, string>, [key, value]: [string, string]): Record<string, string> {
  ...
  private getFullName({
    ...
    return `${chalk.yellow.bold(`${kind ? `${this.getFullType(kind, group)}/` : ''}${name}`)}${
